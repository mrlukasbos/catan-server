...
//        }
    ...
    public ArrayList<Node> getSurroundingNodes(Node n) {
    ...
    public ArrayList<Node> getSurroundingNodes(Edge edge) {
        ArrayList<Node> surroundingNodes = new ArrayList<>();
        for (Node node : nodes) {
            int tilesInCommon = 0;
            for (Tile t : node.getTiles()) {
    ...
    public ArrayList<Edge> getSurroundingEdges(Edge e) {
    ...
    // Tiles are sorted from small to big, and keys are noted from small to big coordinate sums
    public ArrayList<Edge> getSurroundingEdges(Node n) {
    ...
    public void placeVillage(Player p, Node node) {
        node.setPlayer(p);
        node.setStructure(Structure.VILLAGE);
    }

    public void placeCity(Player p, Node node) {
        node.setPlayer(p);
        node.setStructure(Structure.CITY);
    }

    public void placeStreet(Player p, Edge edge) {
        edge.setPlayer(p);
        edge.setRoad(true);
    }

    public void placeStructure(Player p, Structure structure, String key) {
        switch (structure) {
            case STREET:
    ...
    public ArrayList<Node> getStructuresFromPlayer(Structure structureType, Player player) {
        ArrayList<Node> structures = new ArrayList<>();
    ...
    public ArrayList<Edge> getStreetsFromPlayer(Player player) {
    ...
    public boolean nodeIsHarbour(Node node) {
        for (Edge edge : getSurroundingEdges(node) ) {
            if (edge != null && edge.isHarbour()) {
                return true;
            }
    ...
    public boolean playerHasHarbour(Player player) {
    ...
    public boolean playerHasHarbour(Player player, HarbourType harbourType) {
        for (Node node : getCitiesAndVillagesFromPlayer(player)) {
    ...
    public boolean hasEdgeKey(String key) {
    ...
    public boolean hasTileKey(String key) {
